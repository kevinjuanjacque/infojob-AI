...
import {BeakerIcon, BriefcaseIcon, CashIcon, ClockIcon, LocationMarkerIcon, MinusCircleIcon, OfficeBuildingIcon} from "@heroicons/react/outline";
...
6. Competencia en el uso de herramientas tecnológicas`,
  ...
  return (
    <main className="  max-h-full  transition-all ">
      ...
      e.preventDefault();
      e.stopPropagation();
      router.push(`/job/${offer.id}?q=${getModalidadId(offer.teleworking.id)}`)
        ...
        <div className="flex  items-center gap-2 flex-[3]">
          ...
          ></img>
          <div>
            <h1 className="text-sm font-semibold text-[#0A26EE]">{offer.title}</h1>
        ...
        <div className=" flex flex-col justify-between ml-2 flex-[2] overflow-hidden">
          ...
          <p className="w-full mt-2 text-right text-sm  ">
            <CashIcon className="inline-block mr-1" width={15} color="#0A26EE" />
        ...
        <div className=" transition-all ">
        {
           ...
           { !Evaluation.msg && <section className="w-full flex justify-end">

            <Button icon={BeakerIcon} loading={LoadingEvaluation} className=" rounded-full " loadingText="La IA haciendo magia" color="orange" onClick={(e)=>{
                e.stopPropagation();
                setLoadingEvaluation(true)
                onClickCaculateEvaluation()
            }} >  IA Evaluarme </Button></section>}
        </div>
        </section>
      </div>
    </main>
