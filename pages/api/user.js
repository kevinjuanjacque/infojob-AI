...
export default async function User(req,res) {
    ...
    const allSkillExperience = [].concat(...experience.data.experience.map((item)=>item.expertise.map(skill=>skill.skill)));


    const skillExperience = [...new Set(allSkillExperience)];

    const jobs = experience.data.experience.map((item)=>item.job);


    const offer = await axios.get(`https://api.infojobs.net/api/9/offer?q=puesto:"${jobs[0]}"`,{
        headers:{
