import { BeakerIcon, UserAddIcon, VideoCameraIcon } from '@heroicons/react/outline';
import {
  CalendarIcon,
  LocationMarkerIcon,
  BriefcaseIcon,
  ...
  return (
    <main className="bg-fondo min-h-full p-2 ">
      <div
        ...
        className=" rounded-md border-[2px] max-w-4xl border-grayL3 bg-grayL5 p-0 flex flex-row"
        
      >
        <div class="relative rounded-sm border-[1px] border-grayL3  w-[20%] aspect-square hidden md:block">
          <Image
            src={data.profile?.logoUrl ?? 'https://components.infojobs.com/statics/images/pic-company-logo.png'}
      ...
      <Divider />
      <Card decoration="top">
        <Callout title="Requisitos Minimos" className='whitespace-break-spaces' icon={ExclamationIcon} color="yellow">
          {data.minRequirements || 'No se especifican los requisitos minimos'}

          <h6 className='text-md my-2 font-semibold'>Conocimientos requeridos</h6>
          {
            data.skillsList.length>0 ? data.skillsList.map((skill, index) => (
              <Badge color='rose' key={index}>{skill.skill}</Badge>
        ...
        <Button icon={UserAddIcon} color='orange' onClick={()=>{
          //TODO: go to new tab
          window.open(`https://www.infojobs.net/candidate/application/index.xhtml?id_oferta=${data.id}`, '_blank')
        }}> Postular </Button>
        { !Evaluation.msg && 

<Button icon={BeakerIcon} loading={LoadingEvaluation} loadingText="La IA haciendo magia" color="orange" onClick={(e)=>{

    setLoadingEvaluation(true)
    onClickCaculateEvaluation(data.id)
}} >  IA Evaluarme </Button>}
<Button icon={VideoCameraIcon} color='orange' onClick={()=>{
          router.push(`/interview?id=${data.id}`)

        }}> Intentar entrevista con IA </Button>
        </section>
      </Card>
    </main>
  );
